...
import type { BookmarkData, UIMessage } from './types';
...
interface IINADependencies {
  ...
  utils: {
    chooseFile: (
      title: string,
      options?: { chooseDir?: boolean; allowedFileTypes?: string[] },
    ) => string;
...
export class BookmarkManager {
  ...
  constructor(dependencies?: IINADependencies) {
    ...
    this.metadataDetector = new MetadataDetector(this.deps.core as IINACore, this.deps.console, {
      retryAttempts: 3,
      retryDelay: 100,
      cacheTimeout: 30000,
      enableLogging: true,
        ...
        switch (message.type) {
          case 'REQUEST_FILE_PATH': {
            ...
            const responseTarget =
              uiSource === 'overlay'
                ...
                : uiSource === 'sidebar'
                  ? this.deps.sidebar
                  : this.deps.standaloneWindow;
            responseTarget.postMessage(
              JSON.stringify({ type: 'CURRENT_FILE_PATH', data: currentPath }),
            );
            break;
          }
          ...
          case 'UI_READY': {
            ...
            targetUI.postMessage(
              JSON.stringify({ type: 'BOOKMARKS_UPDATED', data: bookmarksForUI }),
            );
            this.deps.console.log(
                ...
                const responseTarget =
                  uiSource === 'overlay'
                    ...
                    : uiSource === 'sidebar'
                      ...
                      : this.deps.standaloneWindow;
                responseTarget.postMessage(
                  JSON.stringify({ type: 'BOOKMARK_DEFAULTS', data: defaults }),
                );
              })
              .catch((error) =>
                this.deps.console.error(
                  `Failed to get bookmark defaults: ${error instanceof Error ? error.message : String(error)}`,
                ),
              );
            break;
  ...
  private loadBookmarks(): void {
    try {
      const { result: stored, duration } = PerformanceUtils.measure(
        'loadBookmarks',
  ...
  public recoverFromBackup(): boolean {
    try {
      const backup = this.deps.preferences.get(`${this.STORAGE_KEY}_backup`) as string;
      if (backup) {
        const parsedBookmarks = JSON.parse(backup) as BookmarkData[];
        this.bookmarks = parsedBookmarks.map((b) => ({
          ...b,
          createdAt: b.createdAt ? new Date(b.createdAt).toISOString() : new Date().toISOString(),
        }));
      ...
      this.deps.console.error(
        `Backup recovery also failed: ${backupError instanceof Error ? backupError.message : String(backupError)}`,
      );
    }
    return false;
  }
  ...
  private setupEventListeners(): void {
    ...
    this.deps.menu.addItem(
      this.deps.menu.item('Export Bookmarks', () => {
        this.handleExportFromMenu().catch((error) =>
          this.deps.console.error(
            `Failed to export bookmarks from menu: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }),
    );
    this.deps.menu.addItem(
      this.deps.menu.item('Import Bookmarks', () => {
        this.handleImportFromMenu().catch((error) =>
          this.deps.console.error(
            `Failed to import bookmarks from menu: ${error instanceof Error ? error.message : String(error)}`,
          ),
        );
      }),
    );
   ...
   */
  public async getBookmarkDefaults(): Promise<{
    title: string;
    description: string;
    tags: string[];
    timestamp: number;
    ...
    try {
      ...
      const metadata = this.generateBookmarkMetadata(currentPath, detectedTitle, currentTime);
  ...
  private generateBookmarkMetadata(
    filepath: string,
    mediaTitle: string,
    timestamp: number,
    userTitle?: string,
  ...
  public jumpToBookmark(id: string): void {
    ...
    try {
      const core = this.deps.core as any;
      if (core.seekTo) {
        core.seekTo(bookmark.timestamp);
      } else if (core.seek) {
      ...
      this.deps.console.warn(`Could not seek to bookmark: ${errorMessage}`);
    }
  }
  ...
  public async handleExportBookmarks(
    options: ExportOptions,
    uiSource: 'sidebar' | 'overlay' | 'window',
  ): Promise<void> {
    try {
      ...
      const bookmarksToExport = this.getFilteredBookmarksForExport(options);
  ...
  private promptForExportPath(format: 'json' | 'csv'): string | null {
    try {
      ...
      const selectedDir = this.deps.utils.chooseFile('Choose folder to save exported bookmarks', {
        chooseDir: true,
      });
      ...
      const filename = this.deps.utils.prompt(
        `Enter filename for export (with .${format} extension):`,
      );
    ...
    if (options.filter) {
      ...
      if (options.filter.tags && options.filter.tags.length > 0) {
        bookmarks = bookmarks.filter(
          (bookmark) =>
            bookmark.tags && bookmark.tags.some((tag) => options.filter!.tags!.includes(tag)),
        );
      }
  ...
  private exportBookmarksToCSV(bookmarks: BookmarkData[], options: CSVOptions): ExportResult {
    try {
      ...
      const fields =
        options.selectedFields.length > 0
  ...
  public async handleImportBookmarks(
    bookmarks: BookmarkData[],
    ...
    try {
      ...
      this.deps.console.log(
        `Import completed: ${importResult.importedCount} imported, ${importResult.skippedCount} skipped, ${importResult.errorCount} errors`,
      );
  ...
  private async importBookmarks(
    ...
    try {
      ...
      const { result: results, duration } = await PerformanceUtils.measureAsync(
        'importBookmarks',
        async () => {
            ...
            async (bookmark, index) => {
              try {
                ...
                if (existingBookmark) {
                  ...
                  switch (options.duplicateHandling) {
                    case 'skip':
                      ...
                      return {
                        type: 'skipped',
                        bookmark,
                        message: `Skipping duplicate: ${bookmark.title}`,
                      };
                  ...
                  // Check maxBookmarks limit before adding new bookmark
                  const maxBookmarks = this.deps.preferences.get('maxBookmarks') || 1000;
                  const limit =
                    typeof maxBookmarks === 'number'
                      ? maxBookmarks
                      : parseInt(maxBookmarks, 10) || 1000;
                  if (this.bookmarks.length >= limit) {
                    skippedCount++;
                    return {
                      type: 'skipped',
                      bookmark,
                      message: `Limit reached, skipping: ${bookmark.title}`,
                    };
                  }
                ...
                return {
                  type: 'unknown',
                  bookmark,
                  message: `Unknown result for: ${bookmark.title}`,
                };
              } catch (error: any) {
                errorCount++;
      ...
      this.deps.console.log(
        `Import batch processing completed in ${duration.toFixed(2)}ms. Results: ${JSON.stringify(resultCounts)}`,
      );
  ...
  private async handleImportFromMenu(): Promise<void> {
    try {
      ...
      if (importResult.success) {
        this.deps.console.log(
          `Import successful: ${importResult.importedCount} bookmarks imported`,
        );
   ...
   */
  private async importBookmarksFromFile(
    filePath: string,
    options: ImportOptions,
  ): Promise<ImportResult> {
    try {
      ...
      this.deps.console.log(
        `Parsed ${bookmarks.length} bookmarks from ${format.toUpperCase()} file`,
      );
  ...
  private parseJSONFile(content: string): BookmarkData[] {
    try {
      ...
      if (Array.isArray(parsed)) {
        ...
        throw new Error(
          "JSON file must contain an array of bookmarks or an object with a 'bookmarks' property",
        );
      }
    } catch (error: any) {
  ...
  private parseCSVFile(content: string): BookmarkData[] {
    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      try {
        ...
        if (values.length !== headers.length) {
          this.deps.console.warn(
            `CSV line ${i + 1} has ${values.length} values but expected ${headers.length}, skipping`,
          );
          continue;
        }
