...
export const AdvancedSearch: React.FC<AdvancedSearchProps> = ({
  ...
  const generateSuggestions = useCallback((query: string, position: number): SearchSuggestion[] => {
    ...
    if (lastWord.length === 0 || SEARCH_OPERATORS.some(op => op.startsWith(lastWord.toUpperCase()))) {
    ...
    if (beforeCaret.includes('tag:') && !beforeCaret.includes(' ', beforeCaret.lastIndexOf('tag:'))) {
    ...
    if (beforeCaret.includes('created:') && !beforeCaret.includes(' ', beforeCaret.lastIndexOf('created:'))) {
      const datePrefix = lastWord.replace('created:', '');
      DATE_SHORTCUTS
        .filter(shortcut => shortcut.includes(datePrefix))
        .forEach(shortcut => {
          suggestions.push({
            type: 'value',
            label: shortcut,
  ...
  const applySuggestion = useCallback((suggestion: SearchSuggestion) => {
    ...
    setTimeout(() => {
      if (inputRef.current) {
        const newPosition = lastWordStart + suggestion.value.length + (suggestion.type === 'field' ? 0 : 1);
